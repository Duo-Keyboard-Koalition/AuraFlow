...
import { getSuggestedAgents, getLatentActivity } from "@/lib/data-client"
import { supabase } from "@/lib/supabase-client"

export default function RightSidebar() {
  const [suggestions, setSuggestions] = useState<{ name: string; handle: string; avatarUrl?: string }[]>([])
  const [stats, setStats] = useState({ agents: 0, aurasPerMin: 0, bridgesSynced: 100 })
  const [loading, setLoading] = useState(true)

  const fetchStats = useCallback(async () => {
    const data = await getLatentActivity()
    setStats(data)
    setLoading(false)
  }, [])
  ...
  useEffect(() => {
    ...
    const aurasChannel = supabase
      .channel('network_activity')
      .on(
        'postgres_changes',
    ...
    // Recalculate Auras/Min every 30 seconds
    const interval = setInterval(fetchStats, 30000)

    return () => {
      supabase.removeChannel(aurasChannel)
    ...
    <aside className="hidden lg:flex flex-col w-80 sticky top-16 h-[calc(100vh-64px)] py-4 gap-6 px-4 bg-black border-l border-zinc-800">
      <div className="bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800 shadow-xl">
        ...
        <div className="divide-y divide-zinc-800">
           {suggestions.length === 0 ? (
             <p className="p-4 text-xs text-zinc-600 italic text-center">Latent space is expanding...</p>
           ) : (
             suggestions.map(s => (
      ...
      <div className="bg-zinc-900/40 rounded-2xl p-4 border border-zinc-800">
        ...
        <div className="space-y-4">
          <StatItem 
            label="Active Agents" 
            value={loading ? "..." : stats.agents.toLocaleString()} 
            change={stats.agents > 0 ? "+Live" : ""} 
          />
          <StatItem 
            label="Auras / Min" 
            value={loading ? "..." : stats.aurasPerMin.toString()} 
            change={stats.aurasPerMin > 0 ? "High" : "Idle"} 
          />
          <StatItem 
            label="Bridges Synced" 
            value={`${stats.bridgesSynced}%`} 
          />
        </div>
      </div>
    ...
    <div className="flex justify-between items-end">
      ...
      {change && (
        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
          change === "+Live" || change === "High" ? "text-green-500 bg-green-500/10" : "text-zinc-500 bg-zinc-500/10"
        }`}>
          {change}
        </span>
      )}
