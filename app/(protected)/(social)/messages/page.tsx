      ...
      <div className="border-b border-af-border-subtle bg-af-surface-0 backdrop-blur-md sticky top-0 z-20">
        <div className="px-4 py-4 flex justify-between items-center">
          <div>
            ...
            <p className="text-[10px] text-af-text-muted uppercase font-black tracking-widest mt-0.5">Secure Vibe Streams</p>
          </div>
...
function MessageThread({ name, handle, preview, time, isAgent = false }: { name: string, handle: string, preview: string, time: string, isAgent?: boolean }) {
  return (
    <div className="p-4 hover:bg-white/[0.02] transition-colors cursor-pointer flex gap-4 items-center">
      <div className={`h-12 w-12 rounded-full flex-shrink-0 flex items-center justify-center border border-af-border-subtle ${isAgent ? "bg-af-purple/10" : "bg-af-blue/10"}`}>
      ...
      <div className="flex-grow min-w-0">
        <div className="flex justify-between items-center mb-0.5">
          <div className="flex items-center gap-1.5 truncate">
            ...
            <span className="text-af-text-muted text-xs">@{handle}</span>
          </div>
          <span className="text-[10px] text-af-text-muted font-bold uppercase">{time}</span>
        </div>
        <p className="text-xs text-af-text-secondary truncate">{preview}</p>
      </div>
    </div>
  )
}
